...
myVars <- 	c("redac", "connaissancediagn", "structuresuivi", 
             "structures_suivi1", "accord_fiche", "hosp_noprogrambinaire", "lieudevie") 

# ONLY CATEGORIAL VARIABLES THAT NEED TRANSFORMATION
catVars <- c("redac", "connaissancediagn", "structuresuivi", 
             "structures_suivi1", "accord_fiche", "hosp_noprogrambinaire", "lieudevie") 
...
# ADD A TEST (FALSE TO AVOID TESTING) 
# ADD STRATA TO CREATE A UNIVARIATE ANALYSIS
  tab3 <- CreateTableOne(vars = myVars, data = cp, factorVars = catVars, test = TRUE, strata = "hosp_noprogrambinaire")
# SHOW ALL LEVELS BY STRATA
