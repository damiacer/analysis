...
addpoly(sav, row=-1, mlab="Meta-Regression Model (Adjusted Effect)")

forest(res, xlim=c(-6.8,3.8), header=TRUE, atransf=exp,
       at=log(c(1/16, 1/8, 1/4, 1/2, 1, 2, 4, 8)), digits=c(2L,4L),
       ilab = cbind(ai, n1i, ci, n2i, weights), ilab.xpos = c(-11.5,-10.5,-8.5,-7.5,-5.5),, ylim=c(-2.5,15))
text(-3.5, 15, "Lattitude", font=2)
abline(h=0)
sav1 <- predict(res)
addpoly(sav1, row=-1, mlab="Meta-Regression Model (Adjusted Effect)")


#- computing the age effect----
...
unifix <- mixmeta(logor, logovar, data)