...
bs$y = bs$ROI0
bs$y = bs$ROI0 - bs$ROI1
empty = lm(y ~ 1, data = bs)

# Sexe 
sex.m = lm(y ~ Sexe, data = bs)
summary(sex.m)
coef(sex.m)
confint(sex.m)
...
table(mubs_oneclass$count)

mubs_oneclass <- as_tibble(mubs_oneclass)
mubs_oneclass <- mubs_oneclass %>% rename(
  #new name = old name
  "time" = "count")
...
m_1 <- lme(ROIr ~ Sexe + Type + StadeJ0 + IMCr + Tabac + Alcool + #Diabète +
                          CTC + IRC + Hyperthyroidie + IPP + PALr + BOR1 + Zometa01,
             random = ~ time | ID, na.action = na.omit, data= mbs)

summary(m_1)
intervals(m_1, which = "fixed")

#-------------------------------------------------------------------------------

# complete case mixed model-------

# duplicating dataset

rep_bs3 <- cbind(bsnew, rep(row.names(bsnew), each = 3))
View(rep_bs3)
names(rep_bs3)

rep_bs3 <- as_tibble(rep_bs3)
rep_bs3 <- rep_bs3 %>% rename(
  #new name = old name
  "ID2" = "rep(row.names(bsnew), each = 3)")
...
mubs3 <- merge(rep_bs3, bs, by.x = "ID", by.y = "ID",
              all.x = TRUE, all.y = FALSE)

str(mubs3$ID)
mubs3$ID = as.numeric(as.character(mubs3$ID))

# create a count variable 
library("sqldf")

mubs3_oneclass <- sqldf("SELECT a.*, COUNT(*) count
              FROM mubs3 a, mubs3 b
              WHERE a.ID = b.ID AND b.ROWID <= a.ROWID
              GROUP BY a.ROWID")
...
table(mbs3$ROI2, useNA = "always")

mbs3$ROI2m <- mbs3$ROI2
mbs3$ROI2m[is.na(mbs3$ROI2m)] <- 1000
mbs3$ROI2m

# excluding patients missing for ROI2

bs3 = mbs3[!(mbs3$ROI2m == 1000),]
dim(mbs3)
dim(bs3)
...
m2 <- lme(ROIr ~ Sexe + Type + StadeJ0 + IMCr + Tabac + Alcool + #Diabète +
             CTC + IRC + Hyperthyroidie + IPP + PALr + BOR1 + Zometa01,
           random = ~ time | ID, na.action = na.omit, data = bs3)
summary(m2)
