...
shapiro.test(ba$Steatose_Qpath)
...
cor.test(ba$Steatose, ba$Steatose_Qpath, method = "pearson")
...
y <- ba$Steatose
x <- ba$Steatose_Qpath
plot(x, y, pch = 20, col = "darkgreen", ylab = "Steatosis", xlab = "Qupath",
     cex = 2.0, cex.axis = 2, cex.lab = 2,
     #xlim = c(-1,10), ylim = c(-1,)
     )
abline(lm(y ~ x), col = "black", lwd = 1)
text(8,37,"y = 0.478 + 0.1785x",srt=37,pos=4, cex = 3)
...
par(xaxs = "i", yaxs = "i", xpd = FALSE, cex.axis = 2.2, cex.lab = 2.2)
plot.roc(roc.st , legacy.axes = TRUE, identity = F, col = "red", lwd = 1,
...
par(xpd = TRUE)
segments(1, 0, 0, 1)
plot(smooth(roc.st), add=T, col="black", lwd = 0.8)
#legend(x=0.30,y=0.10,
  #"bottom", 
#  col=c("red", "black"), legend=c("ROC","Smooth ROC"), lwd=2)
